ARG BUILDER_IMAGE_TAG=vips-8.18.0-go-1.26.1

FROM ghcr.io/cshum/imagor-builder:${BUILDER_IMAGE_TAG} AS builder
...
COPY --from=builder /go/bin/imagorface /usr/local/bin/imagorface
